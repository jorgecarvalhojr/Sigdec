@extends('auth.home')
@section('title', 'Estatísticas de km')
    ...
    <div class="row">
        <div class="col-md-4">
            <table style="text-align: center" border="0" cellpadding="2" cellspacing="0">
                <tr>
                <td colspan="2"><strong>QUILÔMETROS RODADOS REFERENTES AO PERÍODO ENTRE {{$data_inicial}} e {{$data_final}} - ÓRGÃO: {{$nome_orgao}}</strong></td>
                </tr>
                <tr>
                    <td bgcolor="#999999" style="border:solid; border-width:1px; border-color:#000; "><strong>Viatura</strong></td>
                    <td bgcolor="#999999" style="border:solid; border-width:1px; border-color:#000; "><strong>Quilometragem</strong></td>
                </tr>
                @php 
                    $total = 0;
                ...
                @endphp
                @foreach($km as $item)
                    @php $total += $item -> km; @endphp
                    <tr @if($loop -> even) bgcolor="#CCCCCCD" @endif>
                        <td><strong>{{$item -> viatura}}</strong></td>
                        <td><strong>{{$item -> km}}</strong></td>
                    </tr>
                @endforeach
                <tr>
    ...
    <div class="row">
        <div class="col-md-12">
            <table width="100%" style="text-align: center" border="0" cellpadding="2" cellspacing="0">
                <tr>
                    <td colspan="14" align="center"><strong>QUILÔMETROS RODADOS REFERENTES AO ANO DE {{$ano}} - ÓRGÃO: {{$nome_orgao}}</strong> <a href="{{route('export_km', ['data_inicial' => \str_replace('/', '-', $data_inicial), 'data_final' => \str_replace('/', '-', $data_final), 'orgao' => $orgao])}}"><img src="{{asset('storage/icones/excel.png')}}" width="30px"></a></td>
                </tr>
